 ...
 *
 * @uses travel_life_header_style()
 */
function travel_life_custom_header_setup() {
	add_theme_support( 'custom-header', apply_filters( 'travel_life_custom_header_args', array(
		'default-image'          => '%s/assets/uploads/header-image.jpg',
		'default-text-color'     => '181818',
		'width'                  => 1200,
		'height'                 => 528,
		'flex-height'            => true,
		'wp-head-callback'       => 'travel_life_header_style',
	) ) );
	...
	register_default_headers( array(
		'default-image' => array(
			...
			'description'   => esc_html__( 'Default Header Image', 'travel-life' ),
		),
	) );
}
add_action( 'after_setup_theme', 'travel_life_custom_header_setup' );

if ( ! function_exists( 'travel_life_header_style' ) ) :
	/**
	 * Styles the header image and text displayed on the blog.
	 *
	 * @see travel_life_custom_header_setup().
	 */
	function travel_life_header_style() {
		...
		wp_add_inline_style( 'travel-life-style', $css );
	}
endif;
add_action( 'wp_enqueue_scripts', 'travel_life_header_style', 10 );